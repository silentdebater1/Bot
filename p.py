...
# ------------------------------
# Flask Keep-Alive (အရင်ဆုံး ထည့်)
# ------------------------------
app_flask = Flask('')
...
# Default group ID စာရင်းကို ဖိုင်ထဲသို့ စတင်ထည့်
def init_groups():
    save_groups(DEFAULT_GROUPS)
    print(f"{len(DEFAULT_GROUPS)} group IDs ကို {GROUP_FILE} ထဲသို့ စတင်ထည့်ပြီးပါပြီ။")

# Group ID ကို သွင်းဖို့ handler (Bot တွေမှာသုံးရန်)
...
attack_targets = {}
purchase_data = {}
FEATURE_PRICE = 300  # 1 ကြိမ် = 200 Kyat
    ...
    try:
        while attacking_users.get(chat_id):
            for target in list(attacking_users[chat_id]):
                ...
                nickname = None
                user_id = None
                if isinstance(target, int):  # target က user_id ဖြစ်ရင်
                    user_id = target
                    nickname = nicknames.get(user_id, None)

                if nickname and user_id:
                    # nickname ကို clickable mention အနေနဲ့ပြောင်း
                    name_text = f"[{escape_markdown(nickname, version=2)}](tg://user?id={user_id})"
                else:
                    # မရှိရင် normal display_name (clickable if possible)
...
async def timeattack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Count must be a number 1–20")
        return

    if count < 1 or count > MAX_LIMIT:
        await update.message.reply_text("Count must be 1–20")
        return
    ...
    await update.message.reply_text(f"{disp_target} သုံးနိုင်သည့် attack/stop အကြိမ်: {count}")
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("Owner အမိန့်မပါပဲသုံးခြင်တာမရ")
        return
    ...
    for raw_target in context.args:
        ...
        if attacker_lower not in [a.lower() for a in ADMIN_USERNAMES] and attacker_lower != OWNER_USERNAME_LC:
            ...
            if remaining <= 0:
                await update.message.reply_text(f"Attack uses မကျန်တော့ပါ။ /timeattack သုံးပြီး reset လုပ်ပါ")
                continue
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_lower not in [a.lower() for a in ADMIN_USERNAMES] and user_lower != OWNER_USERNAME_LC:
        ...
        if remaining <= 0:
            await update.message.reply_text(f"Stop uses မကျန်တော့ပါ။ /timeattack သုံးပြီး reset လုပ်ပါ")
            return
...
async def hide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("❌ Owner အမိန့်မပါပဲ သုံးလို့မရပါ။")
        return
    ...
    if update.message.reply_to_message:
        ...
        await update.message.reply_text(
            f"🔒 Hidden: {update.message.reply_to_message.from_user.mention_html()}",
            parse_mode="HTML"
        )
    elif context.args:
        try:
            ...
            hidden_targets.add(target_id)
            await update.message.reply_text(f"🔒 Hidden User ID: {target_id}")
        except:
...
async def stop_hide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        await update.message.reply_text(
            f"✅ Unhidden: {update.message.reply_to_message.from_user.mention_html()}",
            parse_mode="HTML"
        )
    elif context.args:
        try:
            target_id = int(context.args[0])
            hidden_targets.discard(target_id)
            await update.message.reply_text(f"✅ Unhidden User ID: {target_id}")
        except:
            await update.message.reply_text("❌ Invalid ID")
    else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user.username
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("❌ Owner အမိန့်မပါပဲ သုံးခွင့်မရှိပါ။")
        return

    help_text = """
📌 *Bot Commands သုံးနည်း*

1️⃣ *`/name`*
📝 Target user နာမည်ချိန်းချင်ရင်:
- `/name 123456789` → ID ဖြင့် target နာမည် update
💡 Bot က target user ရဲ့ အိုင်ဒီနဲ့ name ကို fetch လုပ်ပြီး update လုပ်ပေးပါမယ်။

2️⃣ *`/id`*
🔹 Target user ID သိချင်ရင်:
- `/id @username` → username နဲ့ ID ပြ
- Reply လုပ်ပြီး `/id` → message sender ID ပြ

3️⃣ *`/hide`*
🔒 Target user ကို hide လုပ်ချင်ရင်:
- Reply လုပ်ပြီး `/hide` → Reply message sender hide
- ID ဖြင့်: `/hide 123456789` → ID ဖြင့် hide
🛑 ရပ်ချင်ရင်:
- Reply လုပ်ပြီး `/stophide` → Reply user hide ရပ်
- ID ဖြင့်: `/stophide 123456789` → ID ဖြင့် hide ရပ်

4️⃣ *`/attack`*
⚡ Username / ID နဲ့ attack စတင်:
- `/attack @username`
- `/attack 123456789`
🛑 Stop:
...
5️⃣ *`/hell`*
🔥 ID နဲ့ hell mode စတင်:
- `/hell 123456789`
🛑 Stop: `/stophell 123456789`

6️⃣ *`/funny`*
😂 ID နှစ်ခုဖြင့် funny mode စတင်:
- `/funny 123 456`
🛑 Stop: `/stopfunny 123 456`

7️⃣ *`/show`*
👀 Hidden content / available commands ပြ

8️⃣ *`/speed`*
⏱ Attack speed စစ်:
- `/speed 0.5` → 0.5 စက္ကန့်အလိုက်
"""
